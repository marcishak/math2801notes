...
The following applications are just applications of this rule to discrete and continous probabilites
### Discrete
...
\]
The oppisite is also true for \(f_{Y|X}(y| X = x)\).
...
A Varible is independent if and only if:
\[
f_{Y|X}(y|x) = f_Y(y)
\]
or similarly
...
Cov(X,X) = Var(X)
\]
this one is kinda self explonatry 
